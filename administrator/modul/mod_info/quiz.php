...
$aksi="modul/mod_info/aksi_quiz.php";
...
switch($_GET[act]){
  ...
  default:
      ...
      if ($_SESSION[leveluser]=='admin'){    
        echo "<h2>Informasi Tugas/Quiz</h2><hr>
          <input type=button class='button blue' value='Tambah Pengumuman' onclick=\"window.location.href='?module=info&act=tambahtopikquiz';\">";

        echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>no</th><th>judul</th><th>kelas</th><th>mata kuliah</th><th>tgl buat</th><th>pembuat</th><th>Info</th><th>terbit</th><th>Aksi</th></tr></thead>";
        
        $tampil_topik = mysql_query("SELECT * FROM info_pengumuman ORDER BY id_kelas");
    ...
    while ($r=mysql_fetch_array($tampil_topik)){
      ...
      $tgl_buat   = tgl_indo($r[tgl_buat]);
       echo "<tr><td>$no</td>
             ...
             <td><a href='?module=info&act=edittopikquiz&id=$r[id_info]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /> | 
                         <a href=javascript:confirmdelete('$aksi?module=info&act=hapustopikquiz&id=$r[id_info]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a><br><br>
                 </td>
             </tr>";
      $no++;
    }
    ...
    $tampil_topik = mysql_query("SELECT * FROM info_pengumuman WHERE pembuat = '$_SESSION[idpengajar]'");
        echo "<h2>Daftar Topik Quiz</h2><hr>
          <input class='button blue' type=button value='Tambah Topik' onclick=\"window.location.href='?module=info&act=tambahtopikquiz';\">";
        echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Judul</th><th>Kelas</th><th>Pelajaran</th><th>Tgl Buat</th><th>Info</th><th>Terbit</th><th>Aksi</th></tr></thead>";
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        if (!empty($cek_mapel)){
            echo"<br><b class='judul'>Info / Pengumuman</b><br><p class='garisbawah'></p>
            <table>
            ...
            while ($t=mysql_fetch_array($mapel)){                
                echo "<tr><td>$no</td>
                        ...
                        echo"<td><input type=button class='tombol' value='Lihat Info / Pengumuman'
                       onclick=\"window.location.href='?module=info&act=daftartopik&id=$t[id_matapelajaran]&id_kelas=$data_siswa[id_kelas]';\"></td></tr>";
            $no++;
            }
            echo"</table>";
        }else{
            echo "<script>window.alert('Belum ada pengumuman di kelas anda.');
                    window.location=(href='media.php?module=info')</script>";
        }
    }
    break;
...
case "tambahtopikquiz":
    if ($_SESSION[leveluser]=='admin'){
    echo "
    <form name='form_topik' method=POST action='$aksi?module=info&act=input_topikquiz'>
    <fieldset>
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
    echo "<form name='form_topik' method=POST action='$aksi?module=info&act=input_topikquiz'>
    <fieldset>
    <legend>Tambah Pengumuman</legend>
    <dl class='inline'>
    <dt><label>Judul</label></dt>               <dd> <input type=text name='judul' size='50'></dd>
...
case "edittopikquiz":
    ...
    $edit=mysql_query("SELECT * FROM info_pengumuman WHERE id_info = '$_GET[id]'");
    ...
    echo "<form name='form_topik' method=POST action='$aksi?module=info&act=edit_topikquiz'>
    <input type=hidden name=id value='$t[id_info]'>
    <fieldset>
...
case "daftartopik":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        $topik = mysql_query("SELECT * FROM info_pengumuman WHERE id_kelas = '$_GET[id_kelas]' AND id_matapelajaran = '$_GET[id]' AND terbit='Y'");
        $cek_topik = mysql_num_rows($topik);

        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Info & Pengumuman Tugas / Quiz Mata kuliah $data_mapel[nama]</b><br><p class='garisbawah'></p>
              <table>
              <tr><th>No</th><th>Deskripsi Pengumuman </th><th></th></tr>";
              $no=1;
              while($t=mysql_fetch_array($topik)){
                  ...
                  $waktu = $t[waktu_pengerjaan] / 60;
                  echo"<tr><td rowspan=5>$no</td><td><b>Judul</b></td><td><b>: $t[judul]</b></td></tr>
            ...
            echo "<script>window.alert('Belum ada Pengumuman di mata kuliah ini.');
                    window.location=(href='media.php?module=info')</script>";
        }
    }
    break;
